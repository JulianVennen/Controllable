...
public class RecipeBookOverlay implements IOverlay
{
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, DeltaTracker tracker)
    {
        ...
        if(!tabButtons.isEmpty())
        {
            ...
            graphics.drawString(font, ClientServices.CLIENT.getButtonComponent(ButtonBindings.NEXT_RECIPE_TAB.getButton()), first.getX() + 15 - 5, first.getY() - 13, 0xFFFFFFFF);
            graphics.drawString(font, ClientServices.CLIENT.getButtonComponent(ButtonBindings.PREVIOUS_RECIPE_TAB.getButton()), last.getX() + 15 - 5, last.getY() + last.getHeight() + 13 - 9, 0xFFFFFFFF);
        }
        ...
        {
            graphics.drawString(font, ClientServices.CLIENT.getButtonComponent(ButtonBindings.PREVIOUS_CREATIVE_TAB.getButton()), forwardButton.getX() + 24 - 5, forwardButton.getY() + 4, 0xFFFFFFFF);
        }

        StateSwitchingButton backButton = ((RecipeBookPageAccessor) page).controllableGetBackButton();
        if(backButton.visible)
        {
            graphics.drawString(font, ClientServices.CLIENT.getButtonComponent(ButtonBindings.NEXT_CREATIVE_TAB.getButton()), backButton.getX() - 24 + 12 - 5, backButton.getY() + 4, 0xFFFFFFFF);
        }
    }
}
